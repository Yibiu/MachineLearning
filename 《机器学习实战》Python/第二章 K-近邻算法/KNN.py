...
#
def knn_classify(sample_data, train_data, class_data, k):
    # 计算欧式距离：sqrt[(x1 - x2)^2 + (y1 - y2)^2 + ...]
    row_count = train_data.shape[0]
    diff_data = tile(sample_data, (row_count, 1)) - train_data
    ...
    class_count = {}
    for i in range(k):
        class_type = class_data[sorted_indexes[i]]
...
def movie_run():
    movie_train_data, movie_class_data = create_movie_datas()
...
def file_to_matrix(file_name):
    ...
    for line in lines:
        line = line.strip()
        elems = line.split('\t')
        train_data[index, :] = elems[0:3]
        class_data.append(elems[-1])
        index += 1
...
def dating_run():
    # 加载数据并归一化
    train_dating_data, class_dating_data = file_to_matrix("./datingTestSet.txt")
    normal_data = auto_normalization(train_dating_data)
    ...
    for i in range(test_num):
        class_result = knn_classify(normal_data[i, :], normal_data[test_num:, :],
                                    class_dating_data[test_num:], 3)
        if class_result != class_dating_data[i]:
            error_count += 1.0
        print("the classifier came back with: %s, the real answer is: %s" % (class_result, class_dating_data[i]))
...
def hand_run():
    ...
    train_hand_data = zeros((train_file_num, 1024))
    class_hand_data = []
    for i in range(train_file_num):
        file_name = train_file_list[i].split(".")[0]
        class_type = int(file_name.split("_")[0])
        class_hand_data.append(class_type)
        train_hand_data[i, :] = image_to_vector("trainingDigits/%s" % train_file_list[i])
    ...
    for i in range(test_file_num):
        ...
        class_result = knn_classify(test_vector, train_hand_data, class_hand_data, 3)
